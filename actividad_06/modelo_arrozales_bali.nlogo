...
to setup
  ...
  ask patches [
    ...
    set vecinos_plaga patches at-points vecindad_von_neuwmann radio_plaga false
    set vecinos_cosecha patches at-points vecindad_von_neuwmann radio_cosecha false
  ]
...
to calcular_cosecha
  ;; f_p fracción de vecinos con el mismo itinerario que yo (efecto de plaga)
  let f_p count vecinos_plaga with [ I_t = [I_t] of myself ] / count vecinos_plaga
  ;; f_w fracción de parches en el mundo con el mismo itinerario que yo (efecto del riego)
...
to elegir_siguiente_itinerario
  ifelse regla_decision = "maximo" [
    ...
    ifelse regla_decision = "aleatorio" [
      let vecino_aleatorio one-of vecinos_cosecha
      set I_t1 [I_t] of vecino_aleatorio
    ] [
      ifelse regla_decision = "mayoria" [
        let itinerarios-vecinos [I_t] of vecinos_cosecha
        let modo-itinerario modes itinerarios-vecinos

        set I_t1 one-of modo-itinerario
      ] [
        if regla_decision = "minoria" [
          let itinerarios-minoria filter [i -> count vecinos_cosecha with [I_t = i] = 1] [I_t] of vecinos_cosecha

          ifelse any? itinerarios-minoria [
            set I_t1 one-of itinerarios-minoria
          ] [
            set I_t1 I_t
          ]
        ]
...
to-report vecindad_von_neuwmann [n incluir_centro?]
